...
class StyloAnalyzer(object):
  ...
  @classmethod
  def csv_header(cls):
    return "corpus\tttr\tawl\tanfwl\tawps\tawpp" + "\t" + "\t".join(common_words) + "\t" + "\t".join(punct) + "\ttop"
  ...
  def non_func_word_per_100(self, word: str):
    ...
    return self.non_func_words_freq[word.lower()] / len(self.non_func_words) * 1000

  def __str__(self):
    basic_info = "%s\t%f\t%f\t%f\t%f\t%f" % (self.corpus, self.type_token_ratio, self.av_word_len, self.av_non_func_words_len, self.av_words_per_sent, self.av_words_per_para)
    common_words_info = "\t".join([str(count) for (word, count) in self.common_words_per_1000])
    common_punct_info = "\t".join([str(count) for (token, count) in self.common_puncts_per_1000])
    top_non_func_words = " ".join([word for (word, count) in self.non_func_words_freq.most_common()][:15])
